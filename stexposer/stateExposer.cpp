...
stateExposer::stateExposer()
{
    ...
    stateExposer stateExposer::getInstance(){
	...
	cout << "state at : "<< this.state.timestamp<< endl; 
	for (auto itr : state.list_of_tuples) {
		cout << itr.toString();
	}
	cout << " presented on client : " << state.nodeID<<endl;
    }


    void stateExposer::exposeInfinite(){
    ...
    void stateExposer::exposeWithinAPeriod(long period){
        this.flag = false;// it helps to do that exposeLimit() does nothing when this method is in execution
        int count = 1;
    ...
    static void stateExposer::exposeLimit(){  // method to verify and control that the limits are not exceeded
